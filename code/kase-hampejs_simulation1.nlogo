...
;;*************************************************************
to create-practice  ;; patch procedure
  sprout-practices 1 [    
    set shape "practice"    
    set size .7
    set attraction random 10
...
;; weeks_planned_stay
   ...
   set weeks_spent_inpractice weeks_spent_inpractice + 1
   
   
   if (weeks_spent_inpractice > weeks_planned_stay) [
     move-prophet
   ]
...
to initialize-prophet
  ;; pokud nemam plan, dam si plan (pocatek pobytu v practice)
...
to update-openness 
  ...
  ;; rigidity influence
  ;; changes openness
  if (age > rigidity_effect_start) [set openness (openness - 0.05)] 
...
to-report neighbour-practice
  ...
  if (randomChoice < range3 AND foreign_attraction = 0)[ ask one-of neighbors with [prestige = 3] [  set giver_prestige prestige ask one-of practices-here [ set foreign_attraction attraction ]    ]  show "Go 1"]
  if (randomChoice < (range3 + range2) AND foreign_attraction = 0 AND (count neighbors with [prestige = 2] > 0)) [ ask one-of neighbors with [prestige = 2] [  set giver_prestige prestige ask one-of practices-here [ set foreign_attraction attraction ]    ] show "Go 2"]
  if (randomChoice < (range3 + range2 + range1)  AND foreign_attraction = 0 AND (count neighbors with [prestige = 1] > 0))[ ask one-of neighbors with [prestige = 1] [  set giver_prestige prestige ask one-of practices-here [ set foreign_attraction attraction ]    ]show "Go 3"]
  if (randomChoice < (range3 + range2 + range1 + range0) AND foreign_attraction = 0)[ ask one-of neighbors with [prestige = 0] [  set giver_prestige prestige ask one-of practices-here [ set foreign_attraction attraction ]    ]show "Go 4"]
  
  ] 
  
  if (prestige_influence = 1)[
  ;;PRESTIGE: FIXED 1
  let prestigeSum 0
  ...
  if (prestige_influence = 3)[
  ;;PRESTIGE: ABSOLUTE  
  ask neighbors with-max [prestige][
     set giver_prestige prestige
     ask practices-here with-max [attraction][           
        set foreign_attraction attraction          
     ]
  ]] 
...
423
438
Number of households with practices of given values of attraction
time
...
"" ""
PENS
"a=0" 1.0 0 -1264960 true "" "plot count practices with [attraction = 0]"
"a=1" 1.0 0 -7500403 true "" "plot count practices with [attraction = 1]"
"a=2" 1.0 0 -2674135 true "" "plot count practices with [attraction = 2]"
"a=3" 1.0 0 -955883 true "" "plot count practices with [attraction = 3]"
"a=4" 1.0 0 -6459832 true "" "plot count practices with [attraction = 4]"
"a=5" 1.0 0 -1184463 true "" "plot count practices with [attraction = 5]"
"a=6" 1.0 0 -10899396 true "" "plot count practices with [attraction = 6]"
"a=7" 1.0 0 -13840069 true "" "plot count practices with [attraction = 7]"
"a=8" 1.0 0 -14835848 true "" "plot count practices with [attraction = 8]"
"a=9" 1.0 0 -11221820 true "" "plot count practices with [attraction = 9]"
"a=10" 1.0 0 -16777216 true "" "plot count practices with [attraction = 10]"
...
55
H(a = 0)
count practices with [attraction = 0]
17
1
11
...
113
H(a=1)
count practices with [attraction = 1]
17
1
11
...
173
H(a=2)
count practices with [attraction = 2]
17
1
11
...
Polygon -7500403 true false 75 180 45 195
Polygon -7500403 true false 60 255 165 255
Polygon -7500403 true false 45 240 165 240 45 240
Polygon -1184463 true false 75 120 60 90
Polygon -1184463 true false 30 225 150 225 120 210 105 150 120 120 150 105 165 75 165 60 30 60 30 75 45 105 75 120 90 150 75 210 45 225
Polygon -1184463 true false 105 120 75 105
Polygon -6459832 true false 150 255 225 255 300 150 300 135 285 120 225 120 210 135 150 210 135 240 135 255 150 255
Line -16777216 false 210 165 240 165
Line -16777216 false 225 135 270 135
Line -16777216 false 165 210 210 210
...
<experiments>
  ...
  <experiment name="experiment" repetitions="10" runMetricsEveryStep="false">
    ...
    <metric>count practices with [attraction = 1]</metric>
    <metric>count practices with [attraction = 2]</metric>
    <metric>count practices with [attraction = 3]</metric>
    <metric>count practices with [attraction = 4]</metric>
    <metric>count practices with [attraction = 5]</metric>
    <metric>count practices with [attraction = 6]</metric>
    <metric>count practices with [attraction = 7]</metric>
    <metric>count practices with [attraction = 8]</metric>
    <metric>count practices with [attraction = 9]</metric>
    <metric>count practices with [attraction = 10]</metric>
    <enumeratedValueSet variable="prestige_influence">
      <value value="0"/>
      <value value="1"/>
      <value value="2"/>
      <value value="3"/>
    </enumeratedValueSet>
    <steppedValueSet variable="rigidity_effect_start" first="5" step="5" last="30"/>
    <enumeratedValueSet variable="prophets_limit">
      <value value="0"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="number_of_planned_weeks_for_stay">
      <value value="0"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="high_prestige_closeness">
      <value value="false"/>
    </enumeratedValueSet>
  </experiment>
  <experiment name="experiment_detail" repetitions="50" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="500"/>
    <metric>count turtles</metric>
    ...
    <steppedValueSet variable="rigidity_effect_start" first="0" step="10" last="30"/>
  </experiment>
</experiments>
@#$#@#$#@
@#$#@#$#@
